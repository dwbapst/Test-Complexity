---
title: "K/Pg Range Test Complexity (Publication version)"
author: "Fraass"
date: "7/4/2018"
output:
...
names(foram.dataframe)
names(timeresolution.dataframe)
names(pca.dataframe)
names(hist.bin.dataframe)
names(OAEs.dataframe)
...
occurtot.genus <- 0
occurtot.family <- 0
 contentsa.family.obj <- 0
 contentsb.family.obj <- 0
 contentsc.family.obj <- 0
 contentsd.family.obj <- 0
 contentstot.family.obj <- 0
extin.family <- 0
origin.family <- 0
extin.genus <- 0
origin.genus <- 0
...
contentsa.genus.obj <- 0
contentsb.genus.obj <- 0
contentsc.genus.obj <- 0
contentsd.genus.obj <- 0
contentstot.genus <- 0
...
detach(foram.dataframe)
detach(pca.dataframe)
detach(hist.bin.dataframe)
detach(OAEs.dataframe)
detach(Abbr.dataframe)
detach(sAbbr.dataframe)
detach(Abbr.family.dataframe)
rm(a)
rm(A)
rm(b)
rm(B)
...
All the code to generate this analysis can be found at github.com/Fraass/Test-Complexity. 

## Initial Dataset
```{r diversity w diff timescales, echo = FALSE}
plot(0, 0, 
     type = 'n', 
     xlim = c(170, 0), 
     #xlim = c(80, 50), 
...
```
Fraass et al., 2015 uses a foram biozone binning scheme, while this publication uses a 0.25 myr scheme. The difference between those schemes are stark. In most timeperiods the zonal scheme unsurprisingly inflates the number of individuals. There are a few times that isn't the case, the later part of the Cretaceous, the Danian, the latter stages of the Paleogene, and the later portion of the Miocene. This is much less the case for the genus, which appears much more stable. While this seems obvious, it's an important difference, and demonstrates that binning schemes are much more important in the lower taxonomic levels. 

There are instances (the Danian here, for example) where the additional resolution is useful because of the increased amount of work devoted to understanding the recovery (e.g., Hull and Norris, 2011
 Lowery et al., 2018). With a longer record however, the zone scheme is appropriate.
...
Plot of the included morphometric parameters. `inc` is the set of morphological parameters used to for this study, while `inc2` is the complete dataset. In `inc2` several parameters are `#`'d out, those are used in calculations of other parameters. I use it for the PCA, along with `inc`. Limited covariation for `inc` traits is seen, suggesting that many of these traits are somewhat unrelated to one another. That is not the case with `inc2`, the complete set, as seen below.
...
```
Results of the Principle Components Analysis (PCA) performed on just the `inc` parameters. First, second, and third axes contain ~`r round(sum(PoV[1:3]), 4)*100`% of the variance. 
...
```
Results of a second Principle Components Analysis (PCA) including nearly all morphometric parameters. First, second, and third axes contain ~`r round(sum(PoV2[1:3]), 4)*100`% of the variance. 

```{r biplot2, echo = FALSE}
biplot(res.pca2)
```
The `inc`, more targeted set of parameters lets us look at fewer components with higher variance explained. The number of chamber parameters seem to dominate the first axis in the second kitchen-sink approach. That makes sense, since the number of chambers in the final whorl is dependent on the interchamber angle, which is then summarized by mtheta. So, mtheta, ic1:5 are all covarying, while number of chamber is placed at the other side. Thus, multiple parameters are explaining the same property, and thus the 'kitchen-sink' approach is obviously fundamentally flawed. Axis two is size, (theres also some minor loading of size on A1). Essentially in the 'kitchen-sink' approach the analysis picks out big keeled things (ie., *Globorotalids*) v. others.

Of primary concern is the lack of differentiation on the skree-plot in the 'focused' dataset. That dataset has variance spread throughout various axes. All of our measurements were designed to discriminate between the major groups, and be highly varying. Thus, because of the nature of our measurements the variance should be distributed as we largely did that on purpose. 

## Test Complexity
```{r making test complexity index.1, echo = FALSE}
inc <- c(#"w", 
  #"mtheta", 
  #"lw", 
  #"lh", 
...
build.tci <- morph #morph is the dataset
for(i in inc){
  {morph[h, i]+morph[m, i]}/2->simple.value #averaging holmdelensis and monmouthenesis together
  build.tci[, i] <- morph[, i]-simple.value #subtracking the 'simple.value' from every species
}
for(i in inc){
  abs(build.tci[, i])->build.tci[, i] #taking the absolute value
  decostand(build.tci[, i], method = 'range', na.rm = TRUE)->build.tci[, i] #applying the 'range' to make the individual scores vary from 0 (simple) to 1
}
test.complex.index <- NA #making the object that includes test complexity
for(i in 1:length(build.tci$species)){
  sum(build.tci[i, inc])->test.complex.index[i] #adding all the parameters together
}
```
This is the code that does the bulk of the analysis in this study.
...
```{r making test complexity.5, echo = FALSE}
plot(0, 0, 
     type = 'n', 
     xlim = c(170, 0), 
...
segments(build.tci$origin, 
         ...
         test.complex.index)
```
Plot of test complexity through time, each segment representing an individual species and its geological range.
```{r trophic strategy, echo = FALSE}
...
lines(time.mean(test.complex.index[1:length(test.complex.index) %w/o% c(spin.ind, photo.ind)], 
      ...
      col = 'purple')
```
Break down of trophic levels with running means. The running mean is simply a calculation that examines the first and last occurances and score, and then averages the test complexity for each extant taxon for each interval. Trophic levels are calcuated the same way, only including those taxa within the specified trophic group. Those groups are based on the studies cited within the Methods section.

### Principle Components Analysis as Time Series
```{r comparison with PCA, echo = FALSE}
axis.of.interest <- 1
which(morph$species =="holmdelensis")->h
which(morph$species =="monmouthensis")->m
build.pca <- res.pca$scores[, axis.of.interest] #morph is the dataset

  {build.pca[h]+build.pca[m]}/2->simple.value #averaging holmdelensis and monmouthenesis together
  build.pca <- build.pca-simple.value #subtracking the 'simple.value' from every species

  abs(build.pca)->build.pca #taking the absolute value

plot(0, 0, 
     type = 'n', 
     xlim = c(170, 0), 
     #xlim = c(80, 50), 
     ylim = c(min(build.pca, na.rm = T), 
            max(build.pca, na.rm = T)), 
     xlab = 'Time (Ma)', 
     ylab = "Axis 1")
segments(morph$origin, 
         build.pca, 
         morph$extin, 
         build.pca)
lines(time.mean(build.pca, 
               morph$origin, 
               morph$extin, 0.25), 
      lwd = 2, 
      col = 'black')
```
The results of the PCA as a time series is striklingly similar to our produced plots of test complexity . It is still possible to see the same shape, and view the rough position of the mean values. We chose to employ our Test Complexity Index because of the relative simpilicity of its calculation, despite the slightly more robust analytical approach of the principle components analysis. It also allowed us to talk about the entire collection of disparity and not just certain aspects.
```{r comparison with PCA2, echo = FALSE}
axis.of.interest <- 2
which(morph$species =="holmdelensis")->h
which(morph$species =="monmouthensis")->m
build.pca <- res.pca$scores[, axis.of.interest] #morph is the dataset
...
segments(morph$origin, 
         ...
         build.pca)
```
Each axis, of course, only accounts for a certain aspect of the disparity within the group, and some subsequent axes appear to emphasize the outliers, rather than the bulk trend. 